...
class KBBankPaymentPayTest {
    ...
    @Autowired
    public KBBankPaymentPayTest(Map<String, PaymentService> bankPaymentService, BankAccountRepository bankAccountRepository, RestaurantRepository restaurantRepository, FoodMenuRepository foodMenuRepository, OrderJoinGroupRepository orderJoinGroupRepository, OrderMenuRepository orderMenuRepository, UserRepository userRepository) {
    ...
    void KB_결제_정상작동() {
        ...
        String result = bankPaymentService.get(PaymentCode.KB.getCode()).pay(userId, orderMenu.getId(), bankAccountId, 5000);
    ...
    void KB_결제_아이디_고유번호_다름() {
        ...
        String result = bankPaymentService.get(PaymentCode.KB.getCode()).pay(userId + 1L, orderMenu.getId(), bankAccountId, 5000);
