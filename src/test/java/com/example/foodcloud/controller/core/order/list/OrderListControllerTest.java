...
class OrderListControllerTest {
    ...
    void Get_주문_리스트_출력_정상작동() throws Exception {
        ...
        MockHttpServletRequestBuilder builder = get("/order/list");

        mockMvc.perform(builder)
                .andExpect(status().isOk())
                .andExpect(view().name("thymeleaf/order/list"))
                .andExpect(model().attribute("orderMenuList", orderMenuRepository.findByUserIdFetchJoin(user.getId())));
    }
